    ...
    while True:
        ...
        if not (frameCounter % 10):
            ...
            for (_x, _y, _w, _h) in bikes:
                ...
                for carID in bikeTracker.keys():
                    trackedPosition = bikeTracker[carID].get_position()
                ...
                if matchCarID is None:
                    print('Creating new tracker ' + str(currentBikeID))
                    tracker = dlib.correlation_tracker()
                    tracker.start_track(image, dlib.rectangle(x, y, x + w, y + h))
                    bikeTracker[currentBikeID] = tracker
                    bikeLocation1[currentBikeID] = [x, y, w, h]
                    types[currentBikeID] = "bikes"
                    currentBikeID += 1
